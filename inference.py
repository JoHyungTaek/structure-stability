...
def main():
    ...
    if "unstable_prob" in submission.columns and "stable_prob" in submission.columns:
        ...
        non_id_cols = [c for c in submission.columns if c.lower() != "id"]
        if len(non_id_cols) >= 2:
            # 보통 stable, unstable 순서라면 이렇게
            submission[non_id_cols[0]] = 1.0 - preds
            submission[non_id_cols[1]] = preds
        else:
            raise ValueError("sample_submission.csv 컬럼 구조를 확인하세요.")
